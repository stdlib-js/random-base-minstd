...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( actual.length, 1, 'has expected length' );
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	t.strictEqual( o.name, minstd.NAME, 'has property' );
...
tape( 'if the `state` property is set to a value other than an Int32Array, an error is thrown', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.throws( badValue( values[i] ), TypeError, 'throws a type error when set to '+values[i] );
	}
	t.end();
...
tape( 'attached to the `normalized` method is a method to serialize the generator as a JSON object', function test( t ) {
	...
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, minstd.normalized.NAME, 'has property' );
...
tape( 'the returned function supports setting the generator state to a state array having a different length', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		v1 = rand1();
		t.strictEqual( v1, arr[ i ], 'returns expected value. i: '+i+'.' );
		v2 = rand2();
		t.strictEqual( v2, arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
